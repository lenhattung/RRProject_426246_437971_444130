...
The code and report of our project can be accessed from [repository](RRProject_426246_437971_444130)
...
###   Output of Table 4 (page 1107 in the article) : Result fo the global panel
...
*   Table 4 demonstrates that the null hypothesis of no serial correlation cannot be rejected, given the p-value > 0.05 and absence of a second order of correlation.

*   Table 4 indicates that we employed the Sargan test statistics to assess the validity of overidentifying restrictions.The test yielded a p-value of 1, and thus we fail to reject the null hypothesis that the overidentifying restrictions are valid.

*   The results of the **Sargan** test and **AR(2)** analysis are consistent with the author's findings.

*   In contrast to the author's findings, our panel data model shows that all variables are statistically significant, including Trade.

*   Our model indicates a positive effect of lagged CO2 emissions on current CO2 emissions, whereas the author's findings showed a negative effect of lagged CO2 emissions on current CO2 emissions.

*   Our panel model suggests a positive impact of urban population on CO2 emissions, whereas the author's findings showed a negative impact of urban population on CO2 emissions.

*   Whereas in our panel model the hypothesis is valid for the global panel due to $log(Y)>0\text{ and }log(Y^2) <0$, However, the author's findings show that the hypothesis is invalid. We do have inverted U-shape pattern
